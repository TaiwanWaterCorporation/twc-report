...
public class ResourceDescriptor {
    ...
    private List<ResourceDescriptor> children = new ArrayList<ResourceDescriptor>();	// list of ResourceDescriptors
    private List<ListItem> parameters = new ArrayList<ListItem>();	// list of ListItem

    // This data is used to store the data for sunsequent calls to getQueryData....
    private java.util.List<InputControlQueryDataRow> queryDataCache = null;
    private byte[] data;
    ...
    public void setPropertyMap(Map<String, String> map) {
        ...
        Iterator<Map.Entry<String, String>> ki = map.entrySet().iterator();
        while (ki.hasNext()) {
        	Map.Entry<String, String> k = ki.next();
            rp.getProperties().add(new ResourceProperty(k.getKey(), k.getValue() ) );
        }
