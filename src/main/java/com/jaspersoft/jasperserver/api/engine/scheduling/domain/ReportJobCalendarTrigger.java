...
public class ReportJobCalendarTrigger extends ReportJobTrigger implements
		Serializable {
	...
	/**
	 * Returns the pattern that describes at which minutes (within an hour) the
	 * trigger will fire.
	 * 
	 ...
	 * <ul>
	 * <li>
	 * A single minute value between <code>0</code> and <code>59</code>.</li>
	 * <li>
	 * A minutes range, for instance <code>0-10</code> which means that the
	 * trigger should fire every minute starting from HH:00 to HH:10.</li>
	 * <li>
	 * Minute values and ranges can be concatenated using commas as separators.</li>
	 * <li>
	 * A minute value with an increment, for instance 5/10 which means that the
	 * trigger would fire every 10 minutes starting from HH:05.</li>
	 * <li>
	 * <code>*</code> which means the the job would fire every minute of the
	 * hour.</li>
	 * </ul>
	 * 
	 * @param minutes
	 *            the minutes pattern to be used for the trigger
	 */
	...
	/**
	 * Returns the pattern that describes at which hours (within a day) the
	 * trigger will fire.
	 * 
	 ...
	 * <ul>
	 * <li>
	 * A single hour value between <code>0</code> and <code>23</code>.</li>
	 * <li>
	 * A hours range, for instance <code>8-16</code> which means that the
	 * trigger should fire every hour starting from 8 AM to 4 PM.</li>
	 * <li>
	 * Hour values and ranges can be concatenated using commas as separators.</li>
	 * <li>
	 * A hour value with an increment, for instance 10/2 which means that the
	 * trigger would fire every 2 hours starting from 10 AM.</li>
	 * <li>
	 * <code>*</code> which means the the job would fire every hour.</li>
	 * </ul>
	 * 
	 * @param hours
	 *            the hours pattern to be used for the trigger
	 */
	 ...
	 * 
	 * @param daysType
	 *            one of {@link #DAYS_TYPE_ALL}, {@link #DAYS_TYPE_MONTH} and
	 *            {@link #DAYS_TYPE_WEEK}
	 ...
	 * <ul>
	 * <li>
	 * A single day value between <code>1</code> and <code>31</code>.</li>
	 * <li>
	 * A days range, for instance <code>2-5</code> which means that the trigger
	 * should fire every on each day starting from 2nd to 5th.</li>
	 * <li>
	 * Day values and ranges can be concatenated using commas as separators.</li>
	 * <li>
	 * A day value with an increment, for instance 1/5 which means that the
	 * trigger would fire every 5 days starting on 1st of the month.</li>
	 * <li>
	 * <code>*</code> which means the the job would fire every day.</li>
	 * </ul>
	 * 
	 * @param monthDays
	 *            the month days pattern to be used for the trigger
	 * @see #DAYS_TYPE_MONTH
	 */
	 ...
	 * 
	 * @param months
	 *            the months as <code>java.lang.Byte</code> values
	 */
	 ...
	 * 
	 * @param weekDays
	 *            the week days as <code>java.lang.Byte</code> values
	 */
	 ...
	 * 
	 * @param rjt
	 *            the ReportJob trigger wich compares to this one
	 * @return <code>true</code> if both triggers are equal
	 */
	@Override
	public boolean ifEquals(ReportJobTrigger rjt) {
		ReportJobCalendarTrigger rjct;
		try {
			rjct = (ReportJobCalendarTrigger) rjt;
		} catch (ClassCastException e) {
			// obviously we have different triggers if they are different
			// classes
			return false;
		}
		return super.ifEquals(rjt)
				&& this.minutes.equals(rjct.getMinutes())
				&& this.hours.equals(rjct.getHours())
				&& this.daysType == rjct.getDaysType()
				&& (this.monthDays != null && this.monthDays.equals(rjct
						.getMonthDays()));
	}

	/**
	 * Convenience constructor that returns a distinct copy of the input
	 * ReportJobSource. All of the copy's Object members are themselves copies
	 * as well.
	 * 
	...
	public ReportJobCalendarTrigger(ReportJobCalendarTrigger jobTrigger) {
		...
		if (jobTrigger.getMonths() != null)
			this.setMonths(new TreeSet<Byte>(jobTrigger.getMonths()));
		if (jobTrigger.getWeekDays() != null)
			this.setWeekDays(new TreeSet<Byte>(jobTrigger.getWeekDays()));
	}
